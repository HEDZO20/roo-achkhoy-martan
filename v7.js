...
const V7_DEPARTMENT_PROFILES = [
  {id:'upbringing', code:'ОВР', name:'Отдел воспитательной работы', email:'ruo.ovdo@mail.ru', head:'Начальник отдела не указан', lastLogin:'Не входил', online:false, tasksGiven:0, completed:0, waitingReview:0, overdue:0, avgReview:0, responses:0, rating:100, sourceDirection:'Воспитательная работа'},
  {id:'general', code:'ОБ', name:'Общий отдел', email:'ruo.npo@mail.ru', head:'Начальник отдела не указан', lastLogin:'Не входил', online:false, tasksGiven:0, completed:0, waitingReview:0, overdue:0, avgReview:0, responses:0, rating:100, sourceDirection:'Общий отдел'},
  {id:'methodical', code:'МО', name:'Методический отдел', email:'infometod@bk.ru', head:'Начальник отдела не указан', lastLogin:'Не входил', online:false, tasksGiven:0, completed:0, waitingReview:0, overdue:0, avgReview:0, responses:0, rating:100, sourceDirection:'Методический отдел'},
  {id:'resources', code:'ХО', name:'Хозяйственный отдел', email:'ruo.khg@mail.ru', head:'Начальник отдела не указан', lastLogin:'Не входил', online:false, tasksGiven:0, completed:0, waitingReview:0, overdue:0, avgReview:0, responses:0, rating:100, sourceDirection:'Хозяйственный отдел'},
  {id:'information', code:'ИО', name:'Информационный отдел', email:'roo.inform@mail.ru', head:'Начальник отдела не указан', lastLogin:'Не входил', online:false, tasksGiven:0, completed:0, waitingReview:0, overdue:0, avgReview:0, responses:0, rating:100, sourceDirection:'Информационный отдел'}
];
...
ROLE_USER_MAP.department_head='v7-head-gia';
...
function v7DownloadTemplate(){downloadCSV('shablon_rezultatov_ege_oge_gia.csv',[['ФИО','Школа','Класс','Экзамен','Предмет','Балл','Оценка','Статус','Опоздание, минут','Учебный год'],['Алиев Аслан Русланович','МБОУ «СОШ №1 г. Ачхой-Мартан»','11А','ЕГЭ','Русский язык','82','5','Сдал','0','2025/2026'],['Мусаева Хава Исламовна','МБОУ «СОШ №4 с. Катар-Юрт»','9А','ОГЭ','Математика','67','4','Сдал','12','2025/2026']]);showToast('Шаблон скачан — откройте его в Excel');}
...
function v7DeptTasks(profile){let tasks=state.tasks.filter(t=>t.departmentId===profile.id);if(!tasks.length)tasks=[{title:`Ежемесячная сводка: ${profile.name}`,status:'active',due:'25 июля'},{title:'Контроль ответов образовательных организаций',status:'review',due:'Сегодня'},{title:'Внутреннее поручение начальника РОО',status:'done',due:'20 июля'}];return tasks.slice(0,6);}
function v7DeptActivities(profile){
  const base=[['16:12','Проверен ответ школы и принят отчёт'],['14:40','Создано новое поручение для школ'],['12:18','Сформирована сводная таблица'],['10:05','Получен ответ от образовательной организации'],['09:21','Вход в систему']];
  return base.map((a,i)=>[a[0],i===1?`Создано поручение по направлению «${profile.sourceDirection}»`:a[1]]);
}
function v7RenderDepartments(){
  ...
  const cards=document.getElementById('departmentCards');if(cards)cards.innerHTML=profiles.map(p=>`<article class="v7-department-card ${v7SelectedDepartment===p.id?'active':''}" data-v7-dept="${p.id}"><div class="v7-dept-card-head"><div class="v7-dept-avatar">${p.code}</div><div><h3>${v7Esc(p.name)}</h3><a href="mailto:${v7Esc(p.email)}">${v7Esc(p.email)}</a></div></div><div class="v7-dept-person"><span>${v7Esc(p.head)}</span><strong class="v7-online ${p.online?'':'offline'}">${p.online?'В системе':'Был '+p.lastLogin.toLowerCase()}</strong></div><div class="v7-dept-mini-stats"><div><strong>${p.tasksGiven}</strong><span>создано</span></div><div><strong>${p.completed}</strong><span>завершено</span></div><div><strong>${p.waitingReview}</strong><span>на проверке</span></div><div><strong>${p.overdue}</strong><span>просрочено</span></div></div><div class="v7-dept-card-foot"><div class="v7-dept-score"><b>${p.rating}%</b><span>рейтинг отдела</span></div><button data-v7-dept-open="${p.id}">Подробнее →</button></div></article>`).join('');
