...
function addAppealV3(){const title=prompt('Тема обращения:','Уточнение по поручению');if(!title)return;v3State.appeals.unshift({id:'A-'+(190+v3State.appeals.length),schoolId:state.currentUser?.schoolId||'s1',school:currentSchool()?.name||state.currentUser?.unit||'РОО',title,text:'Новое обращение, созданное в демо-режиме.',status:'new',priority:'Обычное',time:'Только что'});saveV3();renderAppeals();showToast('Обращение зарегистрировано');}
